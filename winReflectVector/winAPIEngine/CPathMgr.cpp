...
/*
	'실행파일이 있는 경로와 실행파일이름까지 문자열'을 얻어오는 window api들이 제공된다. 
	...
	GetModuleFileName
		...
		이를테면 c:/do/it/some.exe 를 바탕화면에 단축 아이콘으로 실행시키면
		c:/do/it/some.exe 이 얻어와진다
...
bool CPathMgr::Create()
{
	...
	{		
		...
		{
			//여기에 실행파일이름을 제거한 순수한 경로만 얻는 코드를 만들자.
