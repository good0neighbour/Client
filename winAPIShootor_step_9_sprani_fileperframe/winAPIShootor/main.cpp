...
/*
    ...
    '스프라이트 애니메이션'의 구현에 대해서 살펴보자.
    sprite animation

        스프라이트 sprite: 작은 조각 이미지, 2D 게임에서 움직이는(애니메이션) 이미지 등을 통칭하는 의미로 쓰자.
        프레임 frame: 스프라이트 애니메이션의 구성 단위 요소.

        애니메이션 animation: 사람의 눈의 잔상 효과를 이용하여 이미지들을 빠르게 순차적으로 보여주면 마치 움직이는 것처럼 보이게 되는 효과를 이용한 영화기법

    스프라이트 애니메이션 sprite animation:
    '각각의 스프라이트 프레임을
    일정 시간 간격으로
    순서대로 차례차례 화면에 출력하는 것'

    i)위에 스프라이트 애니메이션의 정의대로 스프라이트 애니메이션을 테스트해보자.

    사용할 자원의 형태는 다음과 같은 가정을 전제한다
        i) 스프라이트 프레임은 '파일' 단위로 준비한다.
        ii) 스프라이트 프레임들은 모두 같은 크기(너비, 높이)다.
        iii) 스프라이트 파일 이름은 접미사 형태로 '_번호'를 붙인다.

    ii) i)의 내용을 바탕으로 프로그램 구조를 작성하자.
        ...
        CAnimator
            다수개의 애니메이션 시퀀스(CAniSeq)
            애니메이션의 제어 기능(임의의 에니메이션 시퀀스 플레이)

            <--하나의 캐릭터가 다수개의 애니메이션 시퀀스를 가지므로
            사실상 클래스로 추상화하지 않으면
            제작의 난이도가 매우 올라간다
*/
