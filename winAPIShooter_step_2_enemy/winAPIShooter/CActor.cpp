...
//	tBullets[0]->SetVelocity(SVector2D(0.0f, -1.0f) * 200.0f);	//벡터의 스칼라 곱
...
void CActor::DoFire(vector<CBullet*>& tBullets)
{
	...
	tBullets[mCurIndexBullet]->SetVelocity(SVector2D(0.0f, -1.0f) * 200.0f);	//벡터의 스칼라 곱
...
//위치값 겡신이 되어야 하므로 CUnit::Update도 동작이 되어야 한다.
