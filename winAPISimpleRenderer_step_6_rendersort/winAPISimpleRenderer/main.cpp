...
/*
    ...
    이번 스텝에서는 
        ...
        정렬하여 렌더링하는 기능을 추가해보자

        이 기능이 필요한 이유는

        현재 렌더링되고 있는 코드는
        데이터가 나열된 순서로만 렌더링 되고 있다.

        그런데, 임의의 모델이 상황에 따라 카메라에서 어느 것이 더 가까운지( 어느 것이 더 먼지 )가 달라질 수 있기 때문에
        이에 맞춰
        렌더링 순서가 조정되어야 한다.
...
public:
    ...
    virtual void OnUpdate(float tDeltaTime) override
    {
        ...
        //이 목록에 담아둔 삼각형의 정점들의 위치값들로 래스터라이즈할 것이며,
        //래스터라이즈 하기 전에
