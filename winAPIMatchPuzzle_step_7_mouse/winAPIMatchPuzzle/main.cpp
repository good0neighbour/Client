...
private:
    ...
    virtual void OnCreate() override
    {
        ...
        CInputMgr::GetInstance()->AddKey("OnMouseL", VK_LBUTTON);//마우스 왼쪽 버튼 처리 등록
    ...
    virtual void OnUpdate(float tDeltaTime) override
    {
        ...
        //    //마우스 위치값 얻기 api 호출<--Screen 을 기준으로 위치값을 얻는다
        //    GetCursorPos(&tPoint);
        //    //스크린screen 기준의 마우스 위치 정보를 클라이언트client 기준의 위치 정보를 변환한다
        //    ScreenToClient(this->mhWnd, &tPoint);

        //    //테스트 사각형 영역
        //    RECT tRect = { 0, 0, 150, 150 };

        //    OutputDebugString(L"----left button\n");
