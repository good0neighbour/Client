...
/*
    ...
    이번 스텝에서는 
        ...
        Shading 셰이딩( 음영처리 )를 만들어보자.

        여기서는 '면' 단위의 조명에 의한 셰이딩(명암)을 수행하겠다.

        셰이딩의 넓은 의미: 
        최종 색상 결정 = 물체의 색상 + 텍스쳐 + 조명

        'shader': 셰이딩을 하는 작은 조각 프로그램
        'material': (재질) how to shading, 어떻게 세이딩할지를 담은 '데이터'와 '방법'의 단위
