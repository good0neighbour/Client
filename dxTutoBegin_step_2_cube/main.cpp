...
/*
    ...
    i) 정점구조체를 변경, 셰이더 기본 사용

    ii) 상수 버퍼 사용, 변환행렬

        상수 버퍼 constant buffer: 셰이더 측에서 보았을 때 '버퍼' 자체를 바꾸지 않는다는 의미다.

            이를테면, vertex buffer의 경우
                삼각형을 위한 vertex buffer, 사각형을 위한 vertex buffer를 준비해두고
                삼각형을 그리기 위해서는 전자를 사각형을 그리기 위해서는 휴자로 버퍼를 교체해야 하지만
                상수 버퍼의 경우는 상수버퍼를 만들어 놓고 값만 바꾸는 것이다.
...
public:
    virtual void OnCreate() override
    {
        ...
        */
        ...
        //변환행렬들 설정
        //월드변환 행렬
        mMatWorld = XMMatrixIdentity(); //단위행렬로 초기화
    ...
    virtual void OnUpdate(float tDeltaTime) override
    {
        ...
        mMatWorld = XMMatrixRotationY(tTheta);
        ...
        CBTransform tCB;
        tCB.mWorld      = XMMatrixTranspose(mMatWorld);
