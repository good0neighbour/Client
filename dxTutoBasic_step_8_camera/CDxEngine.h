...
//
//
//	// process:프로그램이 실행중인 것. 실행의 흐름  <--n개의 스레드thread로 만들어져 있다.
//	// thread: 실행흐름의 최소단위.
...
//
//			single tread에서 처리해야하는 것들을 다루는 객체이다
//			<-- 주로 자원resources 을 다룬다.
...
//	//window api때 학습한 내용과 비교하여 보자면, 수동으로 작성했던 더블 버퍼 구조는 버퍼 두개가 존재하고 
...
//
//	//랜더링된 결과 이미지 데이터를 담을 수 있는 추상화된 개념이다.
//	//스왑체인의 백버퍼를 가지고 만든다. 
...
//
//
//	//depth buffer를 위해 필요한 메모리
//	//depth stencil view는 임의의 텍스처 메모리(resource)로 취급된다. 
//	ID3D11Texture2D* mpDepthStencil = nullptr;
//
//
//	//깊이버퍼: 깊이값을 기억시켜두는 메모리, 앞뒤판정을 위해 준비되었다. 
//	//스텐실 버퍼: 임의의 효과를 제작하는데 필요한 여분의 메모리. 
//	//DirectX에서는 이 두 버퍼를 하나의 리소스에 담아 두었다. 한 픽셀당 24+8 = 32 비트를 사용한다
