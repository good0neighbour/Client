...
void CMesh::Create(ID3D11Device* tpDevice, LPCWSTR tFileNameModel, LPCWSTR tFileNameTexture)
{
	CreateBuffers(tpDevice, tFileNameModel);

    mpTextureRyu = new CTextureRyu();
    mpTextureRyu->Create(tpDevice, tFileNameTexture);
...
void CMesh::CreateBuffers(ID3D11Device* tpDevice, LPCWSTR tFileNameModel)
{
    ...
    */
    ...
    //---C style 유니코드 문자열을 cpp의 string타입으로 변환 ---
    // LPCWSTR --> wstring --> string
    // 
    //c++ wide string type = c style wide char집합
