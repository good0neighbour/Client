...
public:
    ...
    virtual void OnCreate() override
    {
        ...
        hr = mpD3D->GetD3DDevice()->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), nullptr, &mpVertexShader);
        if (FAILED(hr))
        {
            pVSBlob->Release();
            //return hr;
        ...
        mpMesh = new CMesh();
        mpMesh->Create(mpD3D->GetD3DDevice());
        ...
        hr = mpD3D->GetD3DDevice()->CreateInputLayout(layout, numElements, pVSBlob->GetBufferPointer(),
            pVSBlob->GetBufferSize(), &mpVertexLayout);
        pVSBlob->Release();
//        if (FAILED(hr))
//            return hr;

        mpD3D->GetImmediateDevice()->IASetInputLayout(mpVertexLayout);
        ...
        hr = mpD3D->GetD3DDevice()->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), nullptr, &mpPixelShader);
        pPSBlob->Release();
        //if (FAILED(hr))
          //  return hr;
        ...
        //hr = mpD3D->GetD3DDevice()->CreateBuffer(&bd, &InitData, &mpVertexBuffer);
        ///*if (FAILED(hr))
        //    return hr;*/
        ...
        //mpD3D->GetImmediateDevice()->IASetVertexBuffers(0, 1, &mpVertexBuffer, &stride, &offset);
        ...
        //                hr = mpD3D->GetD3DDevice()->CreateBuffer(&bd, &InitData, &mpIndexBuffer);
        //                /*if (FAILED(hr))
        //                    return hr;*/

        //                mpD3D->GetImmediateDevice()->IASetIndexBuffer(mpIndexBuffer, DXGI_FORMAT_R16_UINT, 0);

        //mpD3D->GetImmediateDevice()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        ...
        bd.CPUAccessFlags   = 0;
        hr = mpD3D->GetD3DDevice()->CreateBuffer(&bd, nullptr, &mpConstantBuffer);
        /*if (FAILED(hr))
            return hr;*/

        hr = CreateDDSTextureFromFile(mpD3D->GetD3DDevice(), L"resources/testUV256by256.dds", nullptr, &g_pTextureRV);
        ...
        hr = mpD3D->GetD3DDevice()->CreateSamplerState(&sampDesc, &g_pSamplerLinear);
    ...
    virtual void OnUpdate(float tDeltaTime) override
    {
        ...
        mpD3D->GetImmediateDevice()->VSSetShader(mpVertexShader, nullptr, 0);
        mpD3D->GetImmediateDevice()->VSSetConstantBuffers(0, 1, &mpConstantBuffer);

        mpD3D->GetImmediateDevice()->PSSetShader(mpPixelShader, nullptr, 0);
        mpD3D->GetImmediateDevice()->PSSetConstantBuffers(0, 1, &mpConstantBuffer);
        //
        mpD3D->GetImmediateDevice()->PSSetShaderResources(0, 1, &g_pTextureRV);
        mpD3D->GetImmediateDevice()->PSSetSamplers(0, 1, &g_pSamplerLinear);
        ...
        mpD3D->GetImmediateDevice()->UpdateSubresource(mpConstantBuffer, 0, nullptr, &cb, 0, 0);
        mpD3D->GetImmediateDevice()->DrawIndexed(36, 0, 0); 


        mpD3D->GetImmediateDevice()->UpdateSubresource(mpConstantBuffer, 0, nullptr, &cb2, 0, 0);
        //mpD3D->GetImmediateDevice()->DrawIndexed(36, 0, 0);
